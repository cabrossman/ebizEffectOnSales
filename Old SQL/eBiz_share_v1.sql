  Select
  A.YEARMONTH, A.ZKEY, A.DOM_TYPE_II, A.DIVISION_NAME, A.CREDIT_BIN, A.OS_HA, A.RN, A.EVER_S2S_CUST,
  ...
  (
    ...
    ELSE 'OVER 75K'
    END CREDIT_BIN, FC.OS_HA, Q.RN, Q.EVER_S2S_CUST, Q.EVER_FOL_CUST, Q.EVER_EBIZ_CUST, BCH.MON, FC.FRIENDLY, Q.CNT_YM,
    ...
    (
      ...
      *
      from  
      (  
        SELECT A.*, ROW_NUMBER() OVER(PARTITION BY A.EVER_S2S_CUST ORDER BY DBMS_RANDOM.RANDOM) AS RN
        FROM
        (
        ...
        /*USED TO FILTER OUT OUTLIERS IN CUSTOMER SAMPLE TO AVOID BIAS IN PARAMETERS*/
        WHERE A.FEI_SLS/A.CNT_YM > 100
        AND TOT_ORDERS/A.CNT_YM > 1
        and A.CNT_YM > 1
      ) X
      /*FILTER OUT CUSTOMERS*/
      where 
      CASE 
      WHEN X.EVER_S2S_CUST = 1 THEN 1
      WHEN X.EVER_FOL_CUST = 1 THEN 1
      WHEN X.EVER_EBIZ_CUST = 0 and X.RN <50001 THEN 1
      ELSE 0
      END > 0
    ...
    WHEN TO_NUMBER(FC.CREDIT_LIMIT) < 75000 THEN '15K TO 75K'
    ELSE 'OVER 75K'
    END, FC.OS_HA, Q.RN, Q.EVER_S2S_CUST, Q.EVER_FOL_CUST, Q.EVER_EBIZ_CUST,BCH.MON,FC.FRIENDLY, BCH.ACCOUNT_NAME, Q.CNT_YM
  ) A